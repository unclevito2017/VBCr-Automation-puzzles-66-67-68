...
def run_vbcr(start_keyspace, end_keyspace):
    ...
    time.sleep(178)  # Wait for 178 seconds
...
while True:
    try:
        while int(end_keyspace, 16) <= int('3ffffffffffffffff', 16):
            # Generate a random 51-bit increment
            increment = ''.join(random.choices('0123456789abcdef', k=13))  # Generate a random 13-character hexadecimal value
            print("Random Increment:", increment)  # Display the randomly chosen increment
            start_keyspace = start_keyspace[:-13] + increment  # Append the increment to the right side of the start keyspace
