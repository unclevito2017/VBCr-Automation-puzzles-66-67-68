...
def load_checkpoint():
    if os.path.exists('checkpoint.pkl'):
        ...
        return '80000000000000000', '80010000000000000'
...
def run_vbcr(start_keyspace, end_keyspace):
    ...
    command = f'VBCr.exe -t 0 -gpu -gpuId 0 -begr {start_keyspace} -endr {end_keyspace} -o {output_filename} -drk 1 -dis 1 -r 70000 -c 1MVDYgVaSN'

    process = subprocess.Popen(command, shell=True, creationflags=subprocess.CREATE_NEW_PROCESS_GROUP)
    time.sleep(250)  # Wait for 250 seconds
...
while int(end_keyspace, 16) <= int('fffffffffffffffff', 16):
    run_vbcr(start_keyspace, end_keyspace)
